...
`synex_groups`, `synex_accounts`, `synex_entities`, `synex_world`, `synex_notify`, `synex_interact`, and `synex_security` are separate Experimental Alpha modules. The current generated source catalog contains 204 versioned contract definitions: 14 are `client-to-server` and the other 190 are server-local. Entities contributes 33 definitions across 32 names because `synex.entities.bucket.create` retains v1 and adds v2. Accounts, Entities and World have no client-callable contract or NUI contract surface. Notify contributes its bounded Action, command-pull and presentation-metric RPCs; Interact contributes nine active-session-fenced discovery, lease, session, graph-acknowledgement and aggregate-metric RPCs. Security contributes eight definitions; only its session- and generation-fenced Sentinel report is client-to-server, while the other seven are server-local. The browser surfaces are transported through `synex_ui`, not through separate NUIs, and no client surface is authority. The separate `synex_control` Alpha discovers bounded read-only Core and domain providers; it is operator tooling, not a gameplay, banking, notification-inbox, interaction-authority, or automatic-enforcement UI. Future financial gameplay/UI belongs to `synex_banking`. None is a supported component of the frozen Core Production-Beta profile.
...
- [Interact architecture, authoring and open acceptance](interact/README.md)
- [Interact World Anchor companion example](../examples/synex_interact_companion/README.md)
...
- [Notify overview and usage boundary](notify/overview.md)
- [Notify architecture and API](notify/architecture.md)
- [Notify lifecycle, queue and progress](notify/lifecycle.md)
- [Notify security and open acceptance](notify/security.md)
- [Security foundation, threat model and open acceptance](security/README.md)
...
- [Capability policy and resource security](architecture/security.md)
