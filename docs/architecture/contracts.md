...
All public contracts in `0.1.0` remain `experimental`. Semantic versions describe compatibility negotiation inside the current source tree; they do not make a contract stable or add a downstream provider to `synex_core` Production-Beta certification. The Groups, Accounts, Entities, World and Security Experimental Alpha contracts, and contracts owned by bridges, examples, or other non-Core components, may change before their separate release decisions.

The current source catalog contains 204 versioned definitions: seven are owned by `synex_core`, one by `synex_example`, 71 by `synex_groups`, 59 by `synex_accounts`, 33 by `synex_entities` across 32 names, seven by `synex_world`, six by `synex_notify`, 12 by `synex_interact`, and eight by `synex_security`. Fourteen declare `client-to-server`: `synex.groups.self.snapshot`, three bounded Notify runtime contracts, nine active-session-fenced Interact discovery/lease/session/graph-acknowledgement/metric contracts, and the internal session-/generation-fenced Security Sentinel report. The other 190 definitions, including all Accounts, Entity and World operations, Interact's three bundle-lifecycle contracts, and Security's seven server integration/read contracts, declare `network: none`. Counts describe this revision and are not a stable ABI promise.
