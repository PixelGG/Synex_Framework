...
Current-tree fresh boot, public API, capacity, prepared/unprepared restart, crash recovery, stale-facade behavior, and the final database-outage/complete-process-recovery run have passed. During the outage Core reached `DEGRADED` with admission closed in 6.73 seconds; after a complete FXServer stop, MariaDB plus a fresh FXServer process returned to `READY` in 10.86 seconds without nonterminal migration state or leaked durable work. Documentation/final-diff/secret review and publication to `main` have also passed. Only the client join/disconnect/reconnect smoke remains open, so the Production-Beta decision remains **NO-GO**.
