...
A dedicated runtime database-health controller gates each ordinary database-backed recurring handler through the current UTC-session probe. Only one coroutine may own a probe; concurrently due gated workers suspend before touching the adapter. A returned failure, adapter exception, missing watchdog, or fixed five-second watchdog expiry removes critical-foundation admission, records only the stable `database-runtime` reason, and moves `READY` to recoverable `DEGRADED`. Clean suspended handlers report `DEGRADED` with `SCHEDULE_SUSPENDED`, while an existing `UNHEALTHY` result is preserved. The connection heartbeat is deliberately exempt and remains active because it owns bounded pending expiry, reservation release, exact-source cleanup, and lease-loss invalidation.
...
Current-tree fresh boot, public API, capacity, prepared/unprepared restart, crash recovery, stale-facade behavior, and the final database-outage/complete-process-recovery run passed. During the outage Core reached `DEGRADED` with admission closed in 6.73 seconds; after a complete FXServer stop, MariaDB plus a fresh FXServer process returned to `READY` in 10.86 seconds without nonterminal migration state or leaked durable work. Documentation/final-diff/secret review, publication to `main`, and the real-client join/clean-disconnect/reconnect smoke also passed. The client run completed both nine-stage connections, retained Doctor `PASS`, and ended with zero open sessions or connection-authority leases. The documented Core-only profile reached Production Beta on 2026-08-25; this is not a stable or framework-wide runtime claim.
