...
A dedicated runtime database-health controller gates each ordinary database-backed recurring handler through the current UTC-session probe. Only one coroutine may own a probe. A concurrently due worker may use the last completed healthy availability result instead of starting another probe; its own adapter failure remains an ordinary worker failure, so synchronized schedules cannot starve or create transient health warnings. Once the controller has marked the database unavailable, gated workers suspend before touching the adapter. A returned probe failure, adapter exception, missing watchdog, or fixed five-second watchdog expiry removes critical-foundation admission, records only the stable `database-runtime` reason, and moves `READY` to recoverable `DEGRADED`. Clean handlers suspended during confirmed unavailability report `DEGRADED` with `SCHEDULE_SUSPENDED`, while an existing `UNHEALTHY` result is preserved. The connection heartbeat is deliberately exempt and remains active because it owns bounded pending expiry, reservation release, exact-source cleanup, and lease-loss invalidation.

When the adapter returns the failure, the owning probe and its tracked database activity complete. Recovery can then proceed after two consecutive successful UTC probes and successful dependency, Core-resource, and persisted-instance reconciliation. The first success deliberately leaves admission closed. An unrelated health reason keeps Core degraded even after database recovery.

The watchdog has a narrower guarantee: it fences admission within five seconds but cannot cancel the outstanding driver promise or Lua `Await`. oxmysql `2.14.1` may reject pool `getConnection()` without invoking the Lua callback, leaving that single tracked probe permanently outstanding. Clearing it or starting superseding probes would hide or multiply uncancelled database activity, so Core remains fail-closed. After MariaDB is restored and independently verified, recovery requires one controlled complete FXServer-process restart; a raw Core restart and repeated Core/FXServer restart loops are not architectural recovery mechanisms.

The frozen accepted Core tree passed fresh boot, public API, capacity, prepared/unprepared restart, crash recovery, stale-facade behavior, and the final database-outage/complete-process-recovery run. During the outage Core reached `DEGRADED` with admission closed in 6.73 seconds; after a complete FXServer stop, MariaDB plus a fresh FXServer process returned to `READY` in 10.86 seconds without nonterminal migration state or leaked durable work. Documentation/final-diff/secret review, publication to `main`, and the real-client join/clean-disconnect/reconnect smoke also passed. The client run completed both nine-stage connections, retained Doctor `PASS`, and ended with zero open sessions or connection-authority leases. That exact Core-only profile reached Production Beta on 2026-08-25. Current Core domain primitives and the Groups Experimental Alpha are later work and do not inherit this runtime evidence.
...
The current post-baseline Core candidate also exposes two owner-bound domain primitives. `api.Database` admits only bounded parameterized operations on tables declared by the immediate caller and fences callbacks to that caller's epoch. A new receipt is claimed and locked per idempotency key before its handler runs; unrelated keys may run in parallel because global and owner capacity rows are locked only after the result is validated. The final receipt transition rechecks caller authority and capacity in the same transaction, while duplicate calls serialize on the receipt and replay its committed result.

`api.DomainDeletions` snapshots registered provider decisions into durable plans and processes pending destructive/anonymizing actions behind fenced leases; stopped or rebound providers cannot inherit an earlier callback token. Provider schema versions cannot change while a pending action still requires the installed version, and plan persistence rechecks the selected version under the provider catalog lock. Retained capacity is 10,000 plans globally and 1,000 per requester. Every plan state remains charged until physical deletion; pending and executing work is never automatically purged. Terminal plans retain their idempotent replay outcome for 30 days, after which bounded compare-and-swap compaction deletes only an unchanged eligible plan, releases both counters, and makes that key reusable. These paths support the Experimental Alpha Groups resource but remain outside the frozen Core acceptance evidence.

Groups binds to Core through a resumable, generation-fenced registration coordinator. A service-health/readiness barrier keeps the resource `UNHEALTHY` until the deletion provider, character participant, service, 71 RPCs, and five workers have registered. Of those RPCs, 70 remain server-only; exactly `synex.groups.self.snapshot` is registered as a client-to-server projection and derives its actor from the current active Core session. Each worker registration retains its scheduler token for exact rollback and resource-stop cleanup. A retryable failure preserves the progress journal and starts a new generation-fenced attempt after five seconds; a non-retryable failure is terminal for that resource epoch and remains fail closed. Automated coverage exercises this binding. On 2026-08-25 the uncommitted working tree also passed live MariaDB, fresh isolated FXServer startup, Groups/Core restart recovery, and Doctor checks. No manual FiveM client self-projection smoke was run, and changed bytes require fresh evidence.

Extension-definition ownership has a second fence inside Groups. Every provider epoch begins with `synex.groups.registries.begin`, which persists the exact Core owner epoch and an advancing synchronization generation before disabling the owner's previous active set. Type, relation-type, duty-state, and attribute-schema registrations require that exact active session. Hydration joins only matching active sessions, while resource-stop cleanup first deactivates and then removes the observed stopped epoch. A delayed registration or older stop therefore cannot cross into a newer provider epoch.
