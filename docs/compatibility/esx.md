...
- caller-bound `ESX.ShowNotification(...)` and `ESX.ShowAdvancedNotification(...)` mapping to bounded local normal Synex toasts;
...
Client shared-object/player-data, callback, and Notify access are separate cataloged surfaces. The coordinator sends bounded allowlists for active consumers that resolve each surface; callback access additionally requires both callback surfaces, while notifications require `esx.client.notification` plus the Notify grant. Missing access clears the corresponding client projection. Direct client exports compare the immediate client resource name with the applicable list, while the repository `es_extended` facade forwards only that immediate caller through a dedicated `*ForConsumer` path. Notification mapping remains `LOCAL`/`toast`/`normal`, plain-text, owner-bound and target-free; caller-opaque ESX notification events are intentionally unsupported. These lists are compatibility API admission, not confidentiality or server-trusted client identity; the client-sent callback consumer is only a routing hint, and callback handlers must still authorize the player action on the server.
