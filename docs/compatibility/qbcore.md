...
- caller-bound `QBCore.Functions.Notify` and historical `Notify` export mapping to a bounded local normal Synex toast;
...
Client `GetCoreObject`/`GetPlayerData` access is also separate from lifecycle. The coordinator sends bounded allowlists only for active consumers that resolve the corresponding client surface; callback functions require both callback surfaces, while `Functions.Notify` requires `qb.client.notification` plus the Notify grant. A callback-only profile is incomplete, and empty access lists clear their projections. Direct exports compare the immediate client resource name with those lists, while the repository `qb-core` facade can forward only that immediate caller through the dedicated `*ForConsumer` path. Notification mapping remains `LOCAL`/`toast`/`normal`, plain-text, owner-bound and target-free; caller-opaque QB notification events are intentionally unsupported. This is compatibility API admission, not confidentiality or server-trusted client identity; the client-sent callback consumer is only a routing hint, and each public callback handler must still authorize the player action on the server.
