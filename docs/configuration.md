...
The Experimental Alpha `synex_notify` resource likewise exposes no server ConVar and owns no database configuration. Canonical payload, queue, history, action, duration, grouping and token-budget bounds are versioned constants in [`shared/limits.lua`](../resources/synex_notify/shared/limits.lua). Capability declaration is not a grant: each consuming resource must request the exact `synex.notify.*` operations it uses and the operator must grant them through the existing Core capability policy. Keep high, critical, banner, broadcast and diagnostics grants narrow. See [Notify rate limits](notify/rate-limits.md) and [Notify security](notify/security.md).

The Experimental Alpha `synex_interact` resource adds one development-only ConVar outside Core configuration. `synex_interact_trace` defaults to `0`. Use `setr synex_interact_trace 1` only on an isolated diagnostic server when bounded client and server interaction trace rings are needed, then restart `synex_interact`. The replicated form is required because both the server and client recorder read the same flag. Frames remain process-local, retention/capacity bounded and redacted; the recorder is disabled and empty by default and is not audit or performance evidence.
