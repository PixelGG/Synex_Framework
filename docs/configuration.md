...
This page is authoritative for `synex_core` configuration only. Entries that expose policy to `synex_accounts` or another non-Core consumer describe an experimental integration point; those resources remain unsupported rework snapshots outside the Core Production-Beta profile.
...
| `retention` | bounded Core audit, terminal outbox/session-control windows, worker cadence and batch size; `financial` is reserved snapshot metadata that Core validates and exposes but does not execute |
...
| `retention.financial.mode` | `retain_forever` | `retain_forever` or `archive` | Reserved for the experimental accounts snapshot; Core only validates and reports it |
| `retention.financial.archiveAfterDays` | `365` | integer `1..36500` | Reserved accounts-snapshot threshold; it schedules no Core-owned worker |
