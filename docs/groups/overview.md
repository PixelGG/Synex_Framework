...
The current source catalog contains **71 Groups contracts**: 70 server-local contracts and one client-to-server self projection. Across the repository's five source catalogs there are currently 171 versioned definitions. Treat those counts as revision-specific; the exact source remains the [generated contract catalog](../../packages/contracts/generated/docs/contracts.md), and the concise cross-resource reference is [Groups API reference](../reference/groups.md).
