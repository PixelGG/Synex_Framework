...
`synex_notify` ist als separate Experimental-Alpha Feedback- und Notification-Orchestration-Engine implementiert. Sie besitzt bounded Lifecycle, Queueing, Prioritäten, Deduplizierung, Gruppierung, Progress, kurzlebige Actions, Owner-/Session-Fences und Rate-Limits; die passive Darstellung nutzt `synex_ui` und stiehlt keinen Focus. Reale FXServer-/CEF-, Restart-, Accessibility-, Controller- und Performance-Abnahmen bleiben offen.
