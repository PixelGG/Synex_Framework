...
- **FUERA DE ALCANCE.** `synex_groups` y `synex_accounts` son engines independientes en Experimental Alpha. `synex_entities` es una Entity Authority Engine solo para servidor en Development / Experimental Alpha; su implementación está presente en el repositorio, pero siguen pendientes las aceptaciones reales de MariaDB, FXServer/OneSync, reinicio/recuperación, clúster, cliente/Control y del candidato exacto. `synex_world` es un recurso World Semantics & Spatial Authority en Development / Experimental Alpha; siguen pendientes sus pruebas reales de MariaDB, FXServer/OneSync, cliente nativo y reinicio. Las proyecciones de dominio de solo lectura en `synex_control` son experimentales. `synex_bridge` está implementado como Compatibility Platform Experimental Alpha fail-closed, pero aún no tiene aceptación real. `synex_ui` está implementado como base de UI en Experimental Alpha; su validación real de FiveM/CEF sigue pendiente. `synex_security` es una base de seguridad observe-first en Experimental Alpha; siguen abiertas las validaciones reales de FXServer/OneSync, MariaDB, reinicio, cliente hostil, gameplay/falsos positivos y rendimiento. Los demás recursos, bibliotecas y ejemplos son snapshots de rework o scaffolds. Ninguno forma parte de la certificación congelada del Core.
...
`synex_groups` es la Organizations Engine en Experimental Alpha. El catálogo actual contiene 71 contratos de Groups y 31 migraciones: 70 contratos son locales del servidor y solo `synex.groups.self.snapshot` es una proyección de cliente limitada y vinculada a la sesión. La evidencia live anterior precede a la migración `032`; las pruebas actuales de cliente, candidato y madurez siguen pendientes. `synex_accounts` es ahora una Financial Engine Experimental Alpha solo para servidor, con 59 contratos locales y 18 migraciones. El árbol de trabajo actual superó el 28-08-2026 el ámbito de base de datos aislado en MariaDB 11.8.8 con 104/104 pruebas; siguen pendientes FXServer, reinicio/recuperación, actualización restaurada, candidato exacto y decisión de madurez. Accounts no tiene superficie cliente ni NUI; el gameplay y la UI pertenecerán posteriormente a `synex_banking`. `synex_entities` es una Entity Authority Engine solo para servidor en Development / Experimental Alpha, con 33 definiciones de contrato versionadas bajo 32 nombres y cuatro migraciones propias. La implementación y las regresiones del repositorio están presentes; siguen pendientes las aceptaciones reales de MariaDB, FXServer/OneSync, reinicio/recuperación, clúster, cliente/Control y del candidato exacto. `synex_world` aporta bundles declarativos, grafo/geometría compilados, contexto espacial, puertas, portales, estado, instancias y una caché de cliente de solo lectura, pero sigue en Development / Experimental Alpha hasta completar las pruebas reales de MariaDB, FXServer/OneSync, cliente nativo y reinicio. Las proyecciones de dominio de solo lectura en `synex_control` están implementadas, pero son experimentales. `synex_bridge` tiene proveedores QB/QBX/ESX separados, pero sigue en Experimental Alpha sin aceptación exacta de FXServer/cliente. `synex_security` aporta ocho contratos, el servicio `synex.security@1` protegido por capabilities, un Sentinel cliente pequeño y tres tablas de evidencia, pero sigue en Experimental Alpha hasta completar las pruebas reales de FXServer/OneSync, MariaDB, reinicio y gameplay/falsos positivos. Los recursos posteriores, demás bibliotecas y ejemplos ejecutables siguen siendo snapshots de rework o scaffolds. Ninguno forma parte de la beta del Core.

`synex_ui` está implementado como una base de UI en Experimental Alpha, con un paquete React de build y un runtime cliente de FiveM separado. Las comprobaciones automatizadas de tipos, unidades/componentes, navegador, build, transporte y estado cerrado no sustituyen las validaciones reales de FiveM/CEF, zona segura, mando, recuperación del foco, legibilidad sobre el juego y rendimiento, que siguen pendientes. La UI no hereda la certificación del Core.

`synex_notify` está implementado como una engine separada de orquestación de feedback y notificaciones en Experimental Alpha. Gestiona ciclo de vida acotado, cola, prioridad, deduplicación, agrupación, progreso, acciones efímeras, fences de owner/sesión y límites de tasa; su presentación pasiva usa `synex_ui` sin robar el foco. Siguen pendientes las validaciones reales de FXServer/CEF, reinicio, accesibilidad, mando y rendimiento.

Esto también incluye los nombres de gameplay reservados `synex_character`, `synex_identity`, `synex_inventory`, `synex_banking`, `synex_phone`, `synex_radio`, `synex_jobs`, `synex_shops`, `synex_vehicles` y `synex_garages`. La presencia de un directorio no demuestra una función terminada. El ciclo de personaje/sesión existente pertenece a `synex_core`.
