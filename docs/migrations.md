...
The Accounts Alpha manifest separately declares 18 ordered migrations, `001_accounts` through `018_access_grant_valid_from_default`. They evolve the original paired ledger into signed 2–16-entry transactions, add reason/currency mint-burn topology, refunds, partial/multiple holds, access policies and restrictions, group-deletion participation, expanded integrity/outbox/control data, V2 archive mirrors, principal-scoped idempotency, and a convergent `valid_from` default for replay-safe access-grant writes.
...
Registered checksum corrections are limited to exact old-to-current portability repairs for `synex_core/021_worker_queue_scalability`, `synex_accounts/011_hold_lifecycle_v2`, `synex_accounts/015_financial_archive_v2`, and `synex_entities/002_entity_lifecycle_authority`. They cover MariaDB metadata, collation, grouped-aggregate, and idempotent named-constraint replay differences without changing a successfully applied domain result. Core accepts an earlier checksum only when the authoritative applied marker proves that exact earlier file completed; its fence and attempt checksum must still be either that earlier value or the registered current value. A failed, applying, indeterminate, unknown, differently identified, or marker-less earlier attempt is never promoted or retried automatically and remains a maintenance-window recovery case.
...
- `synex_accounts` — **Experimental Alpha Financial Engine:** 18 owned migrations for currencies, accounts and owners, reason/topology registries, signed multi-leg ledger entries, reversals/refunds, snapshots, partial/multiple holds, roles/grants/policies/restrictions, integrity/reconciliation, character/group deletion journals, non-destructive V2 transaction/entry archive mirrors, audit, outbox attempts, retry evidence, and replay-safe access-grant defaults;
