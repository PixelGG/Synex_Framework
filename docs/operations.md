...
| `synex_control` | Development / Experimental Alpha NUI; excluded from the Core Beta profile | Provider-discovered, sanitized, summary-first Core/domain diagnostics and exact read-only investigation |
...
While healthy, Core starts a bounded UTC database health probe once per second and gates every ordinary database-backed recurring worker through the same controller. A concurrently due worker uses the last completed healthy availability result rather than starting another probe; it still reports its own adapter error normally. This prevents synchronized workers from starving or oscillating to `DEGRADED` solely because the shared healthy probe yielded. A returned probe failure, a thrown adapter exception, an unavailable watchdog, or a probe still pending after the fixed five-second watchdog sets the `database-runtime` health reason, clears critical-foundation admission, and transitions `READY` to `DEGRADED`; `operational = true` is intentional so diagnostics remain available, while `playerAdmission = false` blocks new joins. Completed outage probes retry after 5, 10, then at most 15 seconds. Once cached availability is false, gated workers suspend without touching the adapter; a previously healthy suspended worker reports `DEGRADED` with `SCHEDULE_SUSPENDED`, while a pre-existing worker failure remains visible instead of being downgraded. The connection heartbeat is deliberately exempt from suspension: it continues bounded pending-TTL, reservation, source, and authority cleanup, and a player or pending attempt whose database lease cannot be renewed may be disconnected or rejected rather than retained with unproven authority.

Recovery has two distinct paths. When oxmysql returns a probe failure or adapter error to Core, that probe completes normally; after MariaDB is restored, the first completed successful probe persists the still-degraded instance state and the second consecutive success must complete dependency refresh, resource-health synchronization, and instance-status reconciliation before the `database-runtime` reason is cleared. Admission opens only if no independent health reason remains. Any failure or exception in that sequence restores the database fence.
...
The control plane is not a remote administration server; it has no state-changing NUI callback and no external HTTP API. Core owns the bounded provider registry, while Groups, Accounts, Entities and Control register their own read models; the experimental Bridge can additionally register the `compatibility` provider. A stopped or failed provider is shown as unavailable rather than queried through a direct table/service fallback.

For isolated Control acceptance, grant `synex.control.view` to the test player and add only the specialist ACEs needed by the scenario: `synex.control.audit`, `synex.control.security`, `synex.control.financial`, and `synex.control.identifiers`. The server rechecks authority for every request and while the panel remains open. `/synex-control` is a player command; console execution cannot open a NUI. Close or revocation releases focus and stops browser refresh work. Overview and provider catalog are separate bounded reads; only those two routes use the short in-process cache, and catalog cache entries are distinct per cursor and limit. Provider cursors are replaced with player/scope-bound 120-second handles, and a Synex resource transition clears cache/cursors before open viewers request the catalog and active view again.

The Node developer CLI's `doctor --bundle` path is a static support artifact, not an in-game Control export or a Cfx console option. The complete artifact is sanitized before hashing and writing. An operator can attach a repository-contained JSON file with `doctor --bundle --runtime-evidence <file>`; the CLI imports and sanitizes it but does not open Control or collect FXServer state itself. Without the option, diagnostics are explicitly `UNAVAILABLE` / `RUNTIME_CONTROL_EVIDENCE_NOT_SUPPLIED` rather than a fabricated provider snapshot. See the [CLI reference](reference/cli.md#diagnose-and-certify).

Control remains Development / Experimental Alpha until the exact FXServer provider lifecycle and CEF focus/resize/keyboard/closed-state acceptance is complete. See [Control plane](reference/control-plane.md) and [Control permissions](control/permissions.md).

### Entities Alpha diagnostics

The checked-in Entity Authority Engine and its optional Control provider remain Development / Experimental Alpha. `synex entities` returns the bounded optional-service summary. `synex doctor entities` pages a read-only diagnostic snapshot containing durable/runtime counts, stale mappings/bindings, duplicate active bindings or namespaced persistent keys, generation/NetID mismatches, persistent runtime orphans, invalid logical owners, stopped-resource leaks, bucket ownership/membership conflicts, component/state schema mismatches, recovery backlog, live-lease conflicts, capacity pressure, current terminal-materialization spawn-failure rate and observed OneSync/state-bag policy. It additionally exposes up to 50 ordered process-local cleanup findings plus queue capacity, total count and truncation; ordinary health/Control summaries expose up to 10. It may recommend review, but it never changes global Cfx ConVars or invokes cleanup.

An unverified native deletion queues bounded retry work, marks Entity health `DEGRADED` as `ENTITY_CLEANUP_PENDING`, and emits a bounded audit record. The scheduled Entity recovery tick retries at most `synex_entities_recovery_batch_size` findings before durable recovery work. Successful retry emits `entities.cleanup_resolved`; repeated failure retains the finding and stable error code. When the last finding is resolved, health returns to `READY` only if the exact cleanup-pending reason is still active. Queue capacity is derived from the configured Entity maximum with a minimum of 64 and maximum of 20,000. Capacity exhaustion rejects the additional enqueue as `UNHEALTHY` / `ENTITY_CLEANUP_QUEUE_EXHAUSTED` and audits that rejection instead of losing it silently.

Do not delete the reported handle manually: Cfx may already have recycled it. The retry path checks the current type/model and available NetID, or the full registered runtime identity, and preserves an unrelated replacement. The queue exists only in the current Lua process; restarting a resource or FXServer is not proof that a leaked native was cleaned. Preserve the bounded diagnostic evidence, stop new Entity-producing activity, and perform controlled live leak inspection and reconciliation before treating the incident as resolved.

When the Alpha provider and Alpha Control NUI are both running, Control discovers the descriptor's 24 bounded Entity views: summary/health/cluster-authority/quota projections; cursor pages for runtime, persistent definitions, bindings, owners, resources, buckets, components, state metadata, recovery history and bucket membership; exact Entity/binding/component/recovery/bucket inspection; exact Entity search; measured metrics; and drift/findings projections. These are read-only source surfaces, not an accepted deployment runbook. Returned handles and NetIDs remain transient; a Cfx network owner is transport state only and never authorization or logical ownership. The outstanding live MariaDB, FXServer/OneSync, restart/recovery and client gates are recorded in the [Entity verification status](reference/entities.md#verification-status), [Entity operations model](entities/overview.md), and [Control acceptance boundary](testing.md#current-control-alpha-boundary).

### Accounts Alpha diagnostics

The exact current command forms are:
...
- When durable events are enabled, Core processes bounded outbox batches and reports dispatch failures. The Experimental Alpha Groups and Accounts resources contain their own domain workers, but those workers are outside Core certification. Domain events are delivered at least once through the capability-gated outbox publication path; consumers deduplicate by stable event ID. Accounts records each publication attempt and moves a row to `dead` after the tenth failed delivery. Generic Core outbox delivery retains the original producer and rechecks that producer's current epoch and manifest declaration. Legacy rows without `producer_resource`, stopped producers, and revoked declarations retry and eventually dead-letter instead of inheriting Core authority. Disabled Core outbox operations fail with `FEATURE_DISABLED`.
- The current Core candidate schedules bounded compaction of expired caller-bound domain-operation receipts and bounded reconciliation of coordinated domain-deletion plans. DataPort handlers for different idempotency keys may execute concurrently: the exact receipt is claimed first, and global/owner capacity rows are locked only after the handler result has been validated. Calls sharing one key serialize and replay the committed result. Provider callbacks are owner-epoch bound; a missing provider rebind or retryable execution error leaves work pending instead of reporting a destructive action as complete.
- Domain-deletion capacity is 10,000 retained plans globally and 1,000 for one requester. Pending, executing, completed, blocked, and failed plans all remain charged until physical purge; pending and executing plans are never purged automatically. A terminal plan preserves its outcome and idempotency key for 30 days. Bounded compare-and-swap compaction then deletes only the same still-eligible terminal version and releases the global/requester counters; after that deletion, the old key may create a new plan. Provider schema upgrades fail closed while pending actions depend on the installed version, and plan persistence rechecks its provider schema under the catalog lock. Do not bypass these guards or manually alter plan/capacity rows. These migration-`027` paths remain outside the frozen Core Production-Beta evidence.
- The Experimental Alpha Groups resource runs bounded outbox, audit-delivery, expiry, creation-request, and deletion-reconciliation work through Core scheduling and its ownership-checked database maintenance port. Registration is resumable and generation-fenced: the Groups service remains `UNHEALTHY` until its deletion provider, character participant, service, 71 RPCs, and five workers pass the readiness barrier. Exactly one RPC, `synex.groups.self.snapshot`, is client-to-server; the other 70 remain server-only. Scheduler tokens provide exact rollback and stop cleanup. Retryable failure resumes the same progress journal through a fresh fenced attempt after five seconds; non-retryable failure is terminal and fail closed for that epoch. Extension owners must additionally call `synex.groups.registries.begin` once per resource start before publishing their complete group-type, relation-type, duty-state, or attribute-schema set; exact owner epochs fence registration, hydration, and stop cleanup. Its Doctor surfaces dead delivery, drift, lifecycle, graph, reference, read-model-cache, and definition-cache findings/metrics. The 2026-08-25 uncommitted working tree passed repository, live-MariaDB, fresh isolated-FXServer, Groups/Core restart, and Doctor checks. The manual client projection smoke, exact committed-revision review, and owner maturity/publication decision remain open; no production support decision exists.
...
Current Core migration `027_domain_primitives` adds the domain-operation receipt capacities and coordinated domain-deletion catalog required by Groups. It installs the 10,000-global/1,000-requester retained-plan limits, terminal `purge_after` queue, counter ownership, and provider-schema lookup needed by the guarded runtime. All plan states consume capacity until bounded terminal purge; do not treat the 30-day terminal retention window as permanent idempotency because the key becomes reusable after the row is physically removed. It is not part of the frozen 26-migration Production-Beta profile. The 2026-08-25 working-tree acceptance applied all 27 Core plus 30 Groups migrations and passed fresh boot, Groups/Core restart recovery, worker registration, and Doctor. Repeat affected gates after runtime bytes change; the manual Groups client-projection smoke and exact committed-candidate review remain open. Do not add Groups to a deployment intended to reproduce the frozen Core profile.
...
Back up and test restore procedures before every deployment that introduces migration files. Never edit a migration already recorded in `synex_schema_migrations` or its attempt history; Synex rejects checksum mismatches except for the exact registered portability corrections listed under [Migration immutability](migrations.md#immutability). A correction is accepted only when its exact earlier checksum already has an authoritative applied marker. Marker-less, `applying`, legacy `failed`, `indeterminate`, unknown, or differently identified states remain fail-closed and require operator reconciliation rather than automatic retry.
