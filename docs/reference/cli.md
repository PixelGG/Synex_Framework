...
The artifact's manual fields are always `NOT_RUN` because this command does not launch FXServer, MariaDB lifecycle drills, a soak harness, or a FiveM client. `NOT_RUN` describes the automated artifact's execution boundary; it must not be rewritten to mirror separate operator evidence. Predecessor `888a7326` passed its exact automated and major server stages, but its planned minimum soak failed at the first hourly outbox-retention execution before the minimum duration completed. The current runtime tree fixes that path through `e0cbf45`; a clean post-documentation revision must receive a newly generated exact automated artifact and server/client evidence. The overall decision is NO-GO. The automated artifact never asserts release readiness. Certification warnings are accepted only from the tool's explicit allowlist, while unknown or semantically different warning classes fail the gate.
