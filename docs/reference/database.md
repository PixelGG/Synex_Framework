...
The account concurrency model expects oxmysql `READ COMMITTED` (`mysql_transaction_isolation_level 2`); Synex does not set that operator ConVar. Verify it in deployment configuration. Account operations use explicit `SELECT ... FOR UPDATE`, sort both public IDs, and acquire each account row and latest-snapshot row through separate statements in that ascending order. Writes use optimistic versions or unique sequence constraints as a second line of defense.
