...
MySQL `8.4` remains a code and migration compatibility target, not a certified Production-Beta database. Multi-instance operation and `kick_old`, `replace_old`, or `allow` duplicate policies are outside this profile. `synex_groups` is Experimental Alpha with separate working-tree evidence and open client/exact-candidate/owner gates. `synex_accounts` is a server-only Experimental Alpha Financial Engine whose current working tree passed the isolated MariaDB 11.8.8 database scope; FXServer, restart/crash-recovery, restored-upgrade, exact-candidate review, and owner maturity/support/publication remain open. `synex_entities` is a separate server-only Development / Experimental Alpha Entity Authority Engine with implementation and repository regression coverage but open fresh MariaDB, live FXServer/OneSync, restart/recovery, cluster, real-client/Control and exact-candidate gates. Its implemented `synex_control` projection is read-only and experimental. `synex_bridge` is an implemented but unaccepted Experimental Alpha Compatibility Platform; exact database, FXServer, facade/restart, mixed-provider and real-client gates remain open. SDK integrations, examples, and later resources remain experimental rework snapshots or scaffolds. None may be advertised as a supported part of the Core beta.

`synex_notify` and `synex_ui` are separate Experimental Alpha foundations and do not inherit this decision. Notify's later candidate gate must exercise exact Core service and client transport, source-generation reuse, owner/UI/Core restart cleanup, capability denials, queue pressure, action replay/expiry, keyboard/gamepad input, live-region behavior, safe-zone placement, gameplay readability, fallback behavior and measured idle/active cost in the real FiveM/CEF runtime.

Interact has a separate [repository and live acceptance gate](interact/testing.md); it is not part of the frozen Core decision.

`synex_security` is a separate observe-first Experimental Alpha foundation and does not inherit this decision. Its focused repository tests and implemented contracts, service, Sentinel, evidence tables, diagnostics, and enforcement policy are not live acceptance. Promotion requires the exact candidate to pass clean MariaDB migration/retention, FXServer/OneSync event and ownership behavior, Security/Core restart and recovery, real-client hostile-input handling, representative gameplay false-positive calibration, and measured runtime-cost gates. Automatic kick and ban remain disabled. See the [Security development and acceptance gate](security/development.md).

Accounts has no client contract or NUI, so its own acceptance does not require a direct FiveM client smoke. Its later release decision must instead exercise the exact 18-migration schema, 59 server-local contract/service operations, financial concurrency and replay, hold/outbox/retention/reconciliation workers, prepared and unplanned restart recovery, restored-copy upgrade/rollback, and final diff/secret review. Any future `synex_banking` or gameplay client surface needs a separate hostile-client and UI gate.
