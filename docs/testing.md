...
| Gate | Current state |
| --- | --- |
| Working-tree automated gate | **PASS (2026-08-28)** — `npm run check`; focused Accounts 61/61; repository Node suite 1,208 passed, 0 failed, 31 gated skips; UI unit tests 66/66; security 339 files / 0 findings; dependency audit 0 vulnerabilities |
...
| Migration declaration | **IMPLEMENTED** — 18 ordered files through `018_access_grant_valid_from_default` |
...
| Dedicated disposable MariaDB working tree | **PASS (2026-08-28)** — 104/104 database-scope tests passed on isolated MariaDB 11.8.8, including all 82 manifest-declared migrations, migration replay, 100-spend, concurrent-hold, randomized-transfer, and commit/outbox-recovery cases; exact-candidate acceptance is still open |
...
[`framework-ci.yml`](../.github/workflows/framework-ci.yml) runs with read-only repository permissions, fully pinned official checkout/setup actions, Node.js 24, and `npm ci`. Its quality job verifies generated consistency, compiles TypeScript, validates manifests and schemas, executes every non-database test scope, runs compatibility and security analysis, certifies the repository, and audits the locked npm graph. It also type-checks, unit-tests, and builds `synex_ui`, then fails when the tracked package or resource-NUI output differs from a clean build. Pull requests use the ordinary `pull_request` event; the workflow does not reference repository secrets or execute through `pull_request_target`.

After the quality job passes, one separate job installs Playwright Chromium on the fixed `windows-2022` runner label and runs the deterministic `synex_ui` Design Lab visual suite, while another starts an isolated MariaDB service, enables the destructive-test gate for its `synex_test_ci` schema, applies every currently owned migration, and checks live schema invariants. The committed visual baselines are owned by that Windows/Chromium target and must be regenerated against the same platform; Linux and Windows can differ in native text and control geometry even with identical browser and font versions. Browser screenshots do not certify FiveM's embedded CEF, and the repository-wide database job does not make experimental resources part of Core certification or broaden the default local test gate.

### Synex UI automated boundary

Run the UI-specific gates from the repository root:
...
The first three commands cover strict TypeScript, unit/component behavior, and the build-time package plus local production NUI. The visual command exercises deterministic Design Lab scenarios in Playwright Chromium across the configured 1080p, 1440p, 4K, 21:9, and 32:9 projects. These checks can detect source/build drift and browser regressions; they do not run the FiveM client, Cfx focus natives, controller input, the embedded CEF build, a moving game framebuffer, or Resmon/GPU measurements. The exact production output still requires the live checklist in [UI development and verification](ui/development.md#fivem-acceptance-gate).

### Synex Notify automated boundary

Run the focused orchestration gate from the repository root:

```bash
npm run test:notify
```

The suite exercises strict DTO/text/container validation, resource and owner-epoch cleanup, exact session/source-generation delivery, capability-gated high/critical/banner/broadcast use, atomic token budgets, bounded send-many behavior, lifecycle and revision fences, progress transitions, action expiry/replay/session ownership, queue arbitration, starvation protection, overflow, deduplication, grouping, quiet contexts, overlay reservations, UI restart reconciliation and bounded spam. UI unit and Playwright suites separately cover the passive Signal Surface, live-region semantics, closed-state transparency, reduced motion/transparency, quality profiles and configured viewports.

These are deterministic headless and Chromium checks. They do not prove Cfx network scheduling, real FiveM input mappings, the embedded CEF accessibility tree, safe-zone placement over gameplay, runtime sound behavior, resource restart under load or Resmon/GPU cost. Those exact-candidate gates remain open and keep Notify at **Experimental Alpha**.

### Synex Interact verification boundary

The [Interact testing guide](interact/testing.md) separates repository gates from the required FXServer/OneSync, real-client, CEF, input, restart, hostile-client and measured-performance gates.

### Synex Security automated boundary

Run the focused Security foundation gate from the repository root:

```bash
npm run test:security
```

The suite exercises canonical signal validation and deduplication, expectation ownership/TTL/revision, independent-root correlation and decay, case lifecycle and bounded persistence projections, enforcement policy and idempotency, Sentinel sequence/freshness/session fences, movement and player-integrity detectors, Cfx guard ports, diagnostics, service/contract boundaries, persistence hardening, and the read-only hardening advisor. The same command runs in Framework CI.

This is deterministic repository evidence for an **Experimental Alpha** implementation. It does not launch FXServer, exercise live OneSync event cancellation/ownership, apply the Security migration to MariaDB, prove Core/resource restart recovery, calibrate false positives against representative gameplay, test a real hostile client, or measure runtime cost. Those gates remain open; automatic kick and ban are disabled. See [Security development and verification](security/development.md).
...
| `ui` | strict TypeScript contracts, runtime/store/transport bounds, representative component and interaction behavior, transparent unmounted closed state, deterministic Design Lab scenarios, package/runtime builds, tracked-output drift, and Chromium visual regression at the configured viewports and accessibility/quality variants; these protect the Experimental Alpha implementation, not FiveM/CEF acceptance |
| `notify` | canonical validation, owner/session/capability fences, bounded lifecycle and queues, rate budgets, priority/fairness, deduplication/grouping, progress revisions/coalescing, action TTL/replay, presentation contexts/reservations, restart reconciliation, diagnostics/manifest/contracts and controlled 1,000/10,000-request pressure; these protect the Experimental Alpha implementation, not FXServer/CEF acceptance |
| `security` | signal/expectation validation and ownership, bounded correlation/cases/evidence, enforcement policy/idempotency, Sentinel fences, detector/guard behavior, persistence projections, diagnostics, service/contracts, fuzz/false-positive fixtures and hardening findings; these protect the Experimental Alpha implementation, not FXServer/OneSync/MariaDB/restart/gameplay acceptance |
...
The `groups`, `accounts`, `entities`, `ui`, `notify`, `security`, `control`, `compatibility`, shared `database`/`stress`, and SDK/tooling integration checks protect current source work. Groups has separate working-tree runtime evidence with remaining client/exact-candidate/owner gates. Accounts has automated implementation and isolated MariaDB 11.8.8 working-tree evidence; FXServer, worker, restart/crash-recovery, restored-upgrade, exact-candidate, and maturity gates remain open. Entities has repository evidence only; fresh MariaDB, FXServer/OneSync, restart/recovery and cluster gates remain open. UI and Notify have automated package/runtime/browser or orchestration evidence only; real FiveM/CEF, focus, controller, safe-zone, gameplay-readability, restart and performance acceptance remain open. Security has focused repository evidence only; exact FXServer/OneSync, MariaDB, restart/recovery, real-client/gameplay calibration and runtime-cost gates remain open. Control has separate provider-runtime and real CEF client gates. Every non-Core pass remains regression evidence rather than a runtime support claim. See [Groups maturity and acceptance](groups/overview.md#maturity-and-acceptance), [Accounts testing boundary](reference/accounts.md#testing-and-acceptance-boundary), [Entity verification status](reference/entities.md#verification-status), [UI development and acceptance](ui/development.md), [Notify development and acceptance](notify/development.md), [Security development and verification](security/development.md), and [Control Alpha boundary](#current-control-alpha-boundary).
...
The World benchmark paths execute the actual Lua spatial-index `queryAt`, `queryNearby` at 10 m and 100 m, coherent Context resolution, registry Anchor/Door resolution and Access modules. They use a fixed deterministic fixture of 50,000 Anchors, 10,000 Zones, 5,000 Doors and 1,000 Locations; World iterations are capped at 5,000 to keep the local run bounded. This embedded run does not execute FXServer scheduling, FiveM natives, OneSync scope/replication, MariaDB I/O/locking, Cfx transport, client rendering, workers or concurrent production load. It is local regression evidence, not a World runtime, capacity, concurrency or production claim.
...
The repository currently does not start FXServer in CI. There is no automated end-to-end Cfx client/server test, external-framework integration environment, FiveM-embedded CEF run, production load test, or multi-instance `kick_old` requester-restart test. The final Core real-client smoke test was therefore manual. Groups still needs its self-projection client smoke. Notify still needs exact FXServer server/client transport, `synex_ui` stop/start, owner restart, keyboard/gamepad action, safe-zone, accessibility and Resmon checks. Security still needs exact FXServer/OneSync guard and Sentinel behavior, live MariaDB migration/retention, resource/Core restart recovery, representative gameplay false-positive calibration, hostile-client testing and measured runtime cost. Accounts has no direct client surface; its current working tree passed the isolated MariaDB scope, but FXServer, worker, prepared/unplanned restart, crash-recovery, restored-upgrade, and exact-candidate evidence remain open. Entities still needs a fresh execution of its gated MariaDB case plus real vehicle/ped/object OneSync lifecycle, resource/Core restart, live cluster/recovery-failure, bucket-player, NetID-reuse and Control CEF/client evidence. Multi-instance and alternate duplicate policies remain outside the supported Beta profile rather than being treated as accepted untested behavior.
