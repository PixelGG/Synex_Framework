...
After the quality job passes, one separate job installs Playwright Chromium and runs the deterministic `synex_ui` Design Lab visual suite, while another starts an isolated MariaDB service, enables the destructive-test gate for its `synex_test_ci` schema, applies every currently owned migration, and checks live schema invariants. Browser screenshots do not certify FiveM's embedded CEF, and the repository-wide database job does not make experimental resources part of Core certification or broaden the default local test gate.
