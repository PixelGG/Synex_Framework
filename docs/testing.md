...
The 2026-08-24 Core acceptance build passed its documented server-side and real-client stages. The current tree subsequently added terminal restart-retry handling, exact migration-026 generated-expression and forced-index-usability verification, and a runtime database-health circuit. The circuit's fail-closed degradation, worker suspension, cleanup continuity, and two-probe recovery have focused headless coverage; the exact combined revision still requires the complete manual FXServer/client and database-outage/recovery sequence. Manual FXServer results are recorded separately from CI because repository automation does not launch FXServer.
...
| Repository certification | **WARN** | Certification completes with unvendored oxmysql runtime-version warnings, capability review warnings, and default-gated live-database skips; it is not a production guarantee |
| Acceptance-build live MariaDB | **PASS ON EARLIER REVISION** | MariaDB 11.8 baseline, 26/26 Core migrations, including migration `026_lease_authority_owner_index`, and the gated database suite; repeat on the exact combined candidate before release |
| Acceptance-build server-only FXServer | **PASS** | Core reached exactly `READY`; caller-bound APIs, Cfx callables, persisted Saga execution, probe restart, prepared Core restart, and next-boot recovery completed |
| Acceptance-build real FiveM client lifecycle | **PASS** | Join, disconnect, reconnect, and a prepared restart with pending database work completed; preparation waited for the tracked database drain before owner teardown |
| Runtime database-health circuit | **LIVE RETEST REQUIRED** | Focused headless tests cover `DEGRADED` with `operational = true`, closed admission, suspended ordinary database workers, deliberately active bounded connection-heartbeat cleanup, and two-success recovery; no live outage PASS is claimed |
| Current combined revision | **MANUAL RETEST REQUIRED** | Repeat the full FXServer/client, database-outage/recovery, restart, and soak sequence before release acceptance |
...
This matrix separates the previously tested acceptance build from the current follow-up changes. A PASS in an earlier row does not transfer to the combined revision. It is not a stable-release, production-load, downstream-resource, external-framework, or deployment-certification claim.
...
The repository currently does not start FXServer in CI. The 2026-08-24 acceptance build completed disposable server-only acceptance and the documented real-client join/disconnect/reconnect plus pending-database-work prepared-restart stages manually. The current combined revision still requires a repeat of that sequence plus the new runtime database-outage/recovery gate. There is no automated end-to-end Cfx client/server test, external-framework integration environment, browser automation run, production load test, or FXServer database-failure chaos environment. A two-instance `kick_old` requester-restart test is also absent, so strict production excludes that mode rather than treating the gap as accepted risk. Run the in-scope checks against the exact deployment before release.
...
> [!IMPORTANT]
> **Current status: partially completed.** A real FiveM client passed join, disconnect, reconnect, and the pending-database-work prepared-restart scenario in the 2026-08-24 acceptance build. Subsequent hardening and production-profile changes, including the runtime database-health circuit, still require this complete manual sequence to be repeated against the final exact revision. The acceptance target is the single-instance `deny_new` profile; the multi-instance duplicate-policy scenario below is retained as future development evidence, not an initial Beta release gate.
...
7. Keep Core running and stop only the verified disposable MariaDB process. Once the next database-health probe starts, require a returned failure or its five-second watchdog to move lifecycle to `DEGRADED` with `operational = true`, admission closed, and the redacted `database-runtime` reason. Ordinary database-backed scheduled workers must suspend; a worker that was already `UNHEALTHY` must retain that failure rather than be downgraded. Confirm the deliberately exempt connection heartbeat still finalizes one expired or failed pending attempt exactly once and returns pending/reservation counts to zero. Let at least three failed probes complete and verify that no retry starts earlier than 5 seconds after the first completed failure, 10 seconds after the second, and the capped 15 seconds after subsequent failures; record normal scheduler delay separately. Restore MariaDB without restarting Core: the first successful probe must leave admission closed, and only the second consecutive success plus instance/dependency reconciliation may return Core to `READY`, persisted instance `ready`, Doctor `PASS`, and workers healthy. Also exercise a deliberately stalled probe: the watchdog must close admission within five seconds of probe start, must not claim query cancellation, and automatic recovery must wait for the outstanding probe to settle.
