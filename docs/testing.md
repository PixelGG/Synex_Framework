...
The 2026-08-24 Core acceptance build passed its documented server-side and real-client stages. The current tree subsequently added terminal restart-retry handling plus exact migration-026 generated-expression and forced-index-usability verification. Those follow-ups passed repository and live-MariaDB regression gates, but the complete manual FXServer/client sequence has not yet been repeated against the combined revision. Manual FXServer results are recorded separately from CI because repository automation does not launch FXServer.
...
| Live MariaDB | **PASS** | MariaDB 11.8 baseline, 26/26 Core migrations, including migration `026_lease_authority_owner_index`, and the gated database suite |
...
| Current combined revision | **MANUAL RETEST REQUIRED** | Post-acceptance restart-retry and migration-verification hardening passed headless and live-database regression gates; repeat the full FXServer/client sequence before release acceptance |
| Two-instance `kick_old` handoff | **NOT TESTED** | The requester-restart case still requires a dedicated two-instance FXServer environment |

This matrix separates the manually tested acceptance build from the current regression-tested follow-up changes. It is not a stable-release, production-load, external-framework, or deployment-certification claim.
...
> [!IMPORTANT]
> **Current status: partially completed.** A real FiveM client passed join, disconnect, reconnect, and the pending-database-work prepared-restart scenario in the 2026-08-24 acceptance build. Two subsequent hardening corrections passed repository and live-database regression gates but still require this complete manual sequence to be repeated against the combined revision. The two-instance `kick_old` requester-restart case remains untested, and the broader checklist below is retained for future release acceptance.
