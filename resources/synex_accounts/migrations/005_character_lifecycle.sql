CREATE TABLE IF NOT EXISTS `synex_account_character_deletions` (
    ...
    PRIMARY KEY (`plan_id`),
    KEY `idx_account_character_deletions_state` (`state`, `created_at`),
    CONSTRAINT `chk_account_character_deletions_plan`
        CHECK (`plan_id` REGEXP '^[A-Za-z0-9_.:-]{8,64}$'),
    CONSTRAINT `chk_account_character_deletions_anonymous_ref`
        CHECK (`anonymous_ref` REGEXP '^[0-9a-f-]{36}$'),
    CONSTRAINT `chk_account_character_deletions_state`
