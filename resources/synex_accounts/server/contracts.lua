...
local function loadDefinitions(rawCatalog, decode)
    if type(rawCatalog) ~= 'string' or #rawCatalog < 2
        ...
        return nil, Foundation.domainError('CONTRACT_CATALOG_INVALID',
            'The Accounts contract catalog is missing or exceeds its bound.')
    end
    if type(decode) ~= 'function' then
        return nil, Foundation.domainError('CONTRACT_CATALOG_INVALID',
            'A JSON decoder is required for the Accounts contract catalog.')
    end
    local decodedOk, decoded = pcall(decode, rawCatalog)
    if not decodedOk then
        return nil, Foundation.domainError('CONTRACT_CATALOG_INVALID',
            'The Accounts contract catalog is not valid JSON.')
    end
    ...
    if not copiedOk then
        ...
        return nil, Foundation.domainError('CONTRACT_CATALOG_INVALID',
            ('The Accounts contract catalog contains an unsupported JSON value (%s).')
                :format(reason), false,
            { stage = 'bounded_copy', reason = reason })
    end
    if type(catalog) ~= 'table' or catalog.schema ~= 1
        or catalog.domain ~= 'synex.accounts' then
        return nil, Foundation.domainError('CONTRACT_CATALOG_INVALID',
            'The Accounts contract catalog has invalid identity metadata.', false,
            { stage = 'identity' })
    end
    if type(catalog.contracts) ~= 'table'
        or #catalog.contracts < 1 or #catalog.contracts > 128 then
        return nil, Foundation.domainError('CONTRACT_CATALOG_INVALID',
            'The Accounts contract catalog has an invalid contract collection.', false,
            { stage = 'contracts' })
    end
    ...
    for index, contract in ipairs(catalog.contracts) do
        if type(contract) ~= 'table' or type(contract.name) ~= 'string'
            or contract.name:sub(1, 15) ~= 'synex.accounts.'
            or contract.provider ~= 'synex_accounts' or contract.kind ~= 'rpc'
            or contract.network ~= 'none' or names[contract.name]
            or type(contract.input) ~= 'table' or type(contract.output) ~= 'table'
            or type(contract.errors) ~= 'table' then
            return nil, Foundation.domainError('CONTRACT_CATALOG_INVALID',
                'The Accounts contract catalog contains an invalid contract.', false,
                { index = index })
        end
